...
describe(getTestFileName(import.meta.url), (): void => {
  ...
  describe(symbolName(normalizeDevice), (): void => {
    ...
    it('derives a missing nodeId from the doc path', (): void => {
      expect(normalizeDevice({ path: '/lab/substrate/pve-01' }).nodeId).toBe('/lab/substrate/pve-01');
    });

    it('normalizes specs and drops edges without a target while defaulting optional fields', (): void => {
  ...
  describe(symbolName(normalizeDevices), (): void => {
    it('normalizes every doc in the list, preserving input order', (): void => {

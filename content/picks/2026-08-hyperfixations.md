---
title: This Month's Hyperfixations
description: What I've been listening to, snacking on, reading, and more this summer.
publishedAt: 2026-08-14
draft: false
edition: August 2026
cards:
  - type: link
    category: food
    size: wide
    title: Trader Joe's Roasted Tomato Labneh Dip
    href: https://www.traderjoes.com/home/products/pdp/roasted-tomato-labneh-dip-081050
    src: /images/picks/2026-08/labneh.png
    alt: A tub of Trader Joe's Roasted Tomato Labneh Dip
    note: A newfound summer staple. Un-freaking-real with sandwiches, pita, and pretty much anything else.
    tags:
      - Cuisine
      - Cooking

  - type: embed
    category: music
    size: wide
    provider: spotify
    url: https://open.spotify.com/track/4t7S2wX7D5SonTHLE5OfIZ?si=def5af424c2347a5
    title: BC, Victoria — Richy Mitch & The Coal Miners
    note: Vibey; feels like driving through Sedona at sunset. A fun one to jam on guitar lately.
    tags:
      - Music
      - Indie

  - type: quote
    category: quotes
    size: wide
    text: The curse of discipline is that every day looks the same. The curse of indiscipline is that every year does.
    attribution: Hemant Mohapatra
    href: https://x.com/mohapatrahemant/status/2086255070585495862
    tags:
      - Quote
      - Stoicism
      - Motivation

  - type: quote
    category: humor
    size: sm
    text: |
      A horse walks into a bar.

      Bartender: "Hey."
      Horse: "Yeah, sure."
    tags:
      - Humor

  - type: link
    category: tech
    size: tall
    title: NFC Tags
    href: https://www.amazon.com/dp/B0F24L4GDM
    src: /images/picks/2026-08/nfc-tags.jpg
    alt: A sheet of small round NFC sticker tags
    note: Having a blast with these little stickers — phone-tap alarm and chore routines, smart-home triggers, and more. A cheap, fun "hacking" project.
    tags:
      - Tech
      - Gadget
      - Product
      - Project

  - type: link
    category: book
    size: wide
    title: The Anthropocene Reviewed — John Green
    href: https://www.penguinrandomhouse.com/books/672554/the-anthropocene-reviewed-by-john-green/
    src: /images/picks/2026-08/anthropocene-reviewed.jpg
    alt: Cover of The Anthropocene Reviewed by John Green
    note: A covid-era gem that I somehow missed. Green's illustrative, witty writing is so thought provoking and entertaining.
    tags:
      - Book
      - Essays
      - Non-Fiction

  - type: embed
    category: podcast
    size: wide
    provider: spotify
    url: https://open.spotify.com/episode/2CxY7DI1ChqKn8MHDZ5cpD?si=4a4a256769ec4f52
    title: 'Huberman Lab: Meditation with Sam Harris'
    note: A great little deep dive into meditation and mindfulness.
    tags:
      - Podcast
      - Mindfulness
      - Health

  - type: embed
    category: video
    size: large
    provider: youtube
    url: https://www.youtube.com/watch?v=tL9Lw250spc
    title: Why does every mammal get ~1 billion heartbeats?
    note: A fascinating dive into the scaling laws of biology. Veritasium is perfect "iPad dinner time" material.
    tags:
      - Science
      - Video
      - Biology
---

Here's what's been living rent-free in my head this month.
